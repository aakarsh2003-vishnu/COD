...
def res2net50_v1b_26w_4s(pretrained=False, **kwargs):
    ...
    if pretrained:
        ...
        for key, value in model_state.items():
            ...
            elif key == "conv1.0.weight" and key in current_state and value.ndim == 4:
                adapted = current_state[key].clone()
                mean_weight = value.mean(dim=1, keepdim=True)
                adapted.copy_(mean_weight.repeat(1, adapted.shape[1], 1, 1) * (3.0 / adapted.shape[1]))
                load_state[key] = adapted
                skipped.append(key + " adapted_to_input_channels")
            else:
                skipped.append(key)
        current_state.update(load_state)
        model.load_state_dict(current_state)
        if skipped:
            print("Res2Net ImageNet skipped/adapted keys:", skipped)
    return model
