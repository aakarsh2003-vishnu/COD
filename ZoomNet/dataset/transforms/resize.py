...
class UniResize(A.DualTransform):
    ...
    def apply(self, img, interpolation=cv2.INTER_LINEAR, **params):
        return resize(img, height=self.height, width=self.width, interpolation=interpolation)
...
def ms_resize(img, scales, base_h=None, base_w=None, interpolation=cv2.INTER_LINEAR):
    ...
    if base_h is None and base_w is None:
        ...
        h = base_h
        w = base_w
    return [resize(img, height=int(h * s), width=int(w * s), interpolation=interpolation) for s in scales]
...
def ss_resize(img, scale, base_h=None, base_w=None, interpolation=cv2.INTER_LINEAR):
    if base_h is None and base_w is None:
        ...
        h = base_h
        w = base_w
    return resize(img, height=int(h * scale), width=int(w * scale), interpolation=interpolation)
