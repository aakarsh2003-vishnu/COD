...
class UniRotate(A.DualTransform):
    ...
    def apply(self, img, angle=0, interpolation=cv2.INTER_LINEAR, **params):
        return _rotate(img, angle, interpolation, self.border_mode, self.value)

    def apply_to_mask(self, img, angle=0, interpolation=cv2.INTER_LINEAR, **params):
        return _rotate(img, angle, cv2.INTER_NEAREST, self.border_mode, self.mask_value)
...
def _rotate(img, angle, interpolation, border_mode, value):
    ...
    return cv2.warpAffine(
        ...
        borderValue=value if value is not None else 0,
    )
