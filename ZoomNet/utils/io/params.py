...
def load_weight(load_path, model: nn.Module):
    ...
    for key, model_value in state_dict.items():
        ckpt_value = ckpt_dict.get(key)
        if ckpt_value is None or ckpt_value.shape == model_value.shape:
            continue
        if key.endswith("conv1.weight") and ckpt_value.ndim == 4 and model_value.ndim == 4:
            adapted_value = model_value.clone()
            copy_channels = min(ckpt_value.shape[1], model_value.shape[1])
            adapted_value[:, :copy_channels] = ckpt_value[:, :copy_channels]
            if model_value.shape[1] > ckpt_value.shape[1]:
                extra_value = ckpt_value.mean(dim=1, keepdim=True)
                adapted_value[:, copy_channels:] = extra_value.repeat(1, model_value.shape[1] - copy_channels, 1, 1)
            ckpt_dict[key] = adapted_value
            print(f"Adapted '{key}' from {tuple(ckpt_value.shape)} to {tuple(model_value.shape)}")
