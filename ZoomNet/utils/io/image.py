...
def read_msi_array(path: str):
    assert path.endswith(".npy"), path
    msi_array = np.load(path)
    assert msi_array.ndim == 3, f"Expected HWC MSI array, got {msi_array.shape}: {path}"
    return msi_array.astype(np.float32)
